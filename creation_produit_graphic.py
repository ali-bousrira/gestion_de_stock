...
class creation_produit_graphic :

    def submit(self):
        # Here you would process the data entered into the fields
        # Check if all fields are filled
        if not self.produit_entrer_nom.get() or not self.produit_entrer_discription.get() or not self.produit_entrer_prix.get() or not self.produit_entrer_quantites.get():
            ...
            if selected_index:  # Check if there is a selection
                ...
                crea_pro = crea_produit()
                crea_pro.crea_final(self.produit_entrer_nom.get(), self.produit_entrer_discription.get(), int (self.produit_entrer_prix.get()), int (self.produit_entrer_quantites.get()), self.data_category[selected_index_number][0])
    ...
    def run (self):
        ...
        produit_text_nom = tk.Label(self.root, text="produit Name")
        ...
        produit_text_prix = tk.Label(self.root, text="Price")
        produit_text_prix.pack()
        self.produit_entrer_prix = tk.Entry(self.root)
        self.produit_entrer_prix.pack()

        produit_text_quantites = tk.Label(self.root, text="quantites")
